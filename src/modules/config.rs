...
/// Replicates existing behavior from TS implementation.
pub fn obj_to_str(config_obj: &Value) -> Result<String> {
    let obj = config_obj.as_object().ok_or_else(|| anyhow::anyhow!("Invalid configObj: Must be a non-empty object."))?;
    ...
    for (section_name, section_val) in obj {
        let subsections = section_val.as_object().ok_or_else(|| anyhow::anyhow!("Invalid section '{}': Must contain subsection objects.", section_name))?;

        // Iterate over subsections
        for (subsection_name, settings_val) in subsections {
            let settings = settings_val.as_object().ok_or_else(|| anyhow::anyhow!("Invalid settings for [{}]: Must be an object.", section_name))?;
